...
f = open('data9201167149709774174.json', 'r')
o = open('melbourne_average.json', 'w')
...
for outerItem in tqdm(result):
    for innerItem in jsonFile['features']:
        if innerItem['properties']['sa1_main16'] == outerItem[8]:
            # if innerItem['properties']['median_age_persons'] != 0:
            temp.append({'cor': [outerItem[4], outerItem[3]], 'weight': innerItem['properties']['median_age_persons']})
            # else:
