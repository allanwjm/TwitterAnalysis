...
sql = """select * from original_adelaide_coordinate"""
cursor.execute(sql)
...
for item in result:
	...
	if sa2Name != 'Adelaide':
		sql = """update original_adelaide_coordinate set sa1_code = '""" + str(resultDict['sa1_maincode_2016']) + """', sa2_code = '""" + str(resultDict['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, Adelaide' where id = """ + str(item[0])
	else:
		sql = """update original_adelaide_coordinate set sa1_code = '""" + str(resultDict['sa1_maincode_2016']) + """', sa2_code = '""" + str(resultDict['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, South Australia' where id = """ + str(item[0])
	try:
		cursor.execute(sql)
		conn.commit()
	except:
