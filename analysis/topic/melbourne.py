...
for item in result:
	...
	if sa2Name != 'Melbourne':
		sql = """update original_melbourne_coordinate set sa1_code = '""" + str(resultDict['sa1_maincode_2016']) + """', sa2_code = '""" + str(resultDict['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, Melbourne' where id = """ + str(item[0])
	else:
		sql = """update original_melbourne_coordinate set sa1_code = '""" + str(resultDict['sa1_maincode_2016']) + """', sa2_code = '""" + str(resultDict['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, Victoria' where id = """ + str(item[0])
	try:
		cursor.execute(sql)
		conn.commit()
	except:
