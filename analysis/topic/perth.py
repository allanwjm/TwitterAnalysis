...
for item in tqdm(result):
	...
	# if str(poly['properties']['sa2_name_2016']) != 'melbourne':
	sql = """update original_perth_coordinate set sa1_code = '""" + str(resultDict['sa1_maincode_2016']) + """', sa2_code = '""" + str(resultDict['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, Perth (WA)' where id = """ + str(item[0])
	# else:
	# 	sql = """update original_melbourne_coordinate set sa1_code = '""" + str(poly['properties']['sa1_maincode_2016']) + """', sa2_code = '""" + str(poly['properties']['sa2_maincode_2016']) + """', sa2_name = '""" + sa2Name + """, Victoria' where id = """ + str(item[0])
	try:
		cursor.execute(sql)
		conn.commit()
	except:
